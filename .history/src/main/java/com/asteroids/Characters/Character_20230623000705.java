...
public abstract class Character {
    ...
    public Character(ImageView ImageView, int x, int y, int width, int height) {
    ...
    public void isInBorder() {
        if (this.character.getTranslateX() < 0) {
            this.character.setTranslateX(this.character.getTranslateX() + GameStateScene.WIDTH);
        }

        if (this.character.getTranslateX() > GameStateScene.WIDTH) {
            this.character.setTranslateX(this.character.getTranslateX() % GameStateScene.WIDTH);
        }

        if (this.character.getTranslateY() < 0) {
            this.character.setTranslateY(this.character.getTranslateY() + GameStateScene.HEIGHT);
        }

        if (this.character.getTranslateY() > GameStateScene.HEIGHT) {
            this.character.setTranslateY(this.character.getTranslateY() % GameStateScene.HEIGHT);
        }
    }
